resource "azurerm_orchestrated_virtual_machine_scale_set" "vmss_terraform_tutorial" {
  ...
  sku_name                    = "Standard_D2s_v3" #Standard_D2s_v3
  instances                   = 2
  ...
  os_profile {
    ...
    custom_data = base64encode(file("vm/user-data.sh"))  # Use custom_data for user data
  }
  ...
  os_disk {
    storage_account_type = "Premium_LRS" #Premium SSD
    caching              = "ReadWrite"
  }
